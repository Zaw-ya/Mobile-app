...
class ClientHeader extends StatelessWidget { // حوليها لـ StatelessWidget لتبسيطها
  final String? title;
  ...
  Widget build(BuildContext context) {
    ...
    // نستخدم BlocBuilder ليقوم بإعادة بناء الـ Badge فقط عند تغير الـ state
    return BlocBuilder<NotificationsCubit, NotificationsStates>(
      builder: (context, state) {
        final int count = state.unreadCount; // الرقم يأتي من الـ Cubit مباشرة
              ...
              GestureDetector(
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    // الـ Badge يظهر الآن بناءً على الـ count القادم من الـ state
                    if (count > 0) 
                      Positioned(
                        bottom: -6,
...
//     return Container(
//       padding: EdgeInsets.only(top: edge * 2.5, bottom: edge, left: edge, right: edge),
//       decoration: const BoxDecoration(color: AppColor.homeBackground),
//       child: Row(
//         mainAxisAlignment: MainAxisAlignment.spaceBetween,
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
...
//                   builder: (context, state) {
//                     final count = notificationCount ?? state.unreadCount;
